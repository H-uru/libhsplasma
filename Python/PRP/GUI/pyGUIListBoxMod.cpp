...
PY_PLASMA_TYPE_INIT(GUIListElement) {
    ...
    if (PyType_Ready(&pyGUIListElement_Type) < 0)
        return NULL;
...
PY_PROPERTY(plKey, GUIListBoxMod, scrollControl, getScrollCtrl, setScrollCtrl)

static PyGetSetDef pyGUIListBoxMod_GetSet[] = {
    pyGUIListBoxMod_scrollControl_getset,
    PY_GETSET_TERMINATOR
};
...
PY_PLASMA_TYPE_INIT(GUIListBoxMod) {
    ...
    if (PyType_Ready(&pyGUIListBoxMod_Type) < 0)
        return NULL;
